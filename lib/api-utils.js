...
export const makeSlackFilePublic = async (fileId) => {
  return await (await fetch('https://slack.com/api/files.sharedPublicURL', {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${process.env.SLACK_BOT_TOKEN_MAKE_PUBLIC}`,
      'Content-Type': 'application/json'
    },
    body: JSON.stringify({
      file: fileId
